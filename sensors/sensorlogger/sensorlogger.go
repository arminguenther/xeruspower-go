...
const (
	...
	STATE_BYPASS_SELECTED_BUT_INACTIVE int32 = 31 // Bypass selected but inactive
	STATE_I1_ACTIVE                    int32 = 32 // Inlet 1 active
	STATE_I2_ACTIVE                    int32 = 33 // Inlet 2 active
	STATE_BYPASS_ACTIVE                int32 = 34 // Operational state: bypass active
)
...
type Logger interface {
	...
	// Retrieve log records for a peripheral device slot.
	//
	//	@param recs       Result: Sensor log records
	...
	// Retrieve log records with timestamps for a peripheral device slot.
	//
	//	@param recs       Result: Sensor log records
	//	@param slot       Peripheral device slot reference
	//	@param recid      First record id
	...
	//
	//	@param slot    Peripheral device slot to be checked
	//
...
// Sensor log backup state
type LoggerBackupState int

const (
	STOPPED   LoggerBackupState = iota // sensor log backup is stopped
	RUNNING                            // sensor log backup is running
	SUSPENDED                          // sensor log backup is suspended (because of an error)
)
...
// Event: Sensor logger backup state has changed
type LoggerBackupStateChangedEvent interface {
	event.Event
	BackupState() LoggerBackupState // Sensor log backup state
	isLoggerBackupStateChangedEvent()
}

// Event: A sensor logger backup failure occured
type LoggerBackupFailureEvent interface {
	event.Event
	ErrMsg() string // Error message
	isLoggerBackupFailureEvent()
}
